...
with infos:
    ...
    st.write(
        'AWI ist ein Analysetool für die Immobilienwertermittlung und bietet dir umfangreiche Analysemöglichkeiten '
        'rund um die Bewertung deiner Immobilie.')
    st.write('---')
    ...
    st.write(
        'Immobilienbewertungstools gibt es wie Sand am Meer :palm_tree: Doch AWI ist keine gewöhnliche App zur reinen '
        'Bewertung von Immobilien. Um eine fundierte Entscheidung in Hinblick auf den Kauf bzw. Verkauf einer '
        'Immobilie zu treffen, sind tiefgreifende Informationen zur eigenen Immobilie notwendig. Im Gegensatz zu '
        'herkömmlichen Tools, bietet AWI diese Informationen. Neben einer aussagekräftigen Immobilienbewertung, '
        'die AWI mit Hilfe eines intelligenten Machine Learning Ansatzes ermittelt, bietet AWI weiterführende '
        'Analysen, um eine möglichst umfangreiche Immobilienbetrachtung zu ermöglichen. AWI bietet beispielsweise '
        'eine exakte Beschreibung welchen Einfluss die von dir hinterlegten Faktoren auf den Immobilienpreis nehmen. '
...
def user_input_features():
    ...
    weitere_details = st.beta_expander('weitere Details')
    with weitere_details:
        col1, col2 = st.beta_columns(2)
        ...
        with col1:
            ...
            immobilienart = st.selectbox('Immobilienart', (
                'Doppelhaushälfte', 'Einfamilienhaus', 'Etagenwohnung', 'Sonstige', 'Mehrfamilienhaus',
                'Erdgeschosswohnung', 'Dachgeschosswohnung', 'Zweifamilienhaus', 'Wohnung', 'Villa', 'Reihenmittelhaus',
                'Reihenendhaus', 'Bungalow', 'Maisonette', 'Apartment', 'Reiheneckhaus', 'Penthouse', 'Unbekannt'))
            heizung = st.selectbox('Art der Heizung', (
                'Zentralheizung', 'Etagenheizung', 'Fußbodenheizung', 'Ofenheizung', 'Sonstige', 'Unbekannt'))
        ...
        with col2:
            ...
            immobilienzustand = st.selectbox('Immobilienzustand', (
                'Neuwertig', 'Altbau', 'Erstbezug', 'Gepflegt', 'Modernisiert', 'Neubau', 'Projektiert', 'Sonstige',
                'Teil- oder vollrenovierungsbedürftig', 'Teil- oder vollsaniert', 'Teil- oder vollrenoviert',
                'Unbekannt'))
            energietyp = st.selectbox('Energietyp', (
                'Gas', 'Fernwärme', 'Holz', 'Luft-/Wasserwärme', 'Öl', 'Pellets', 'Solar', 'Sonstige', 'Strom',
                'Unbekannt'))

    # Eingabefeld 3 in Expander (Gäste_WC, barrierefrei, Aufzug,...)
    weitere_konfigurationen = st.beta_expander('spezifische Angaben')
    with weitere_konfigurationen:
...
# Codierung und Zuordnung
    ...
    immobilienzustand_string = 'SELECT immobilienzustand_targetenc FROM Encoding_immobilienzustand WHERE ' \
    ...
    energie_effizienzklasse_string = 'SELECT energie_effizienzklasse_targetenc FROM Encoding_energie_effizienzklasse ' \
                                     'WHERE energie_effizienzklasse=\'' + energie_effizienzklasse + '\''
    energie_effizienzklasse = np.float32(
        pd.read_sql_query(energie_effizienzklasse_string, con=db_connection).iloc[0][0])

    # Zuordnung der Eingabe-Features
    data = {'plz': plz, 'immobilienart': immobilienart, 'immobilienzustand': immobilienzustand,
            'barrierefrei': barrierefrei, 'terrasse_balkon': terrasse_balkon, 'unterkellert': unterkellert,
            'vermietet': vermietet, 'energietyp': energietyp, 'heizung': heizung, 'gaeste_wc': gaeste_wc,
            'energie_effizienzklasse': energie_effizienzklasse, 'aufzug': aufzug,
            'anzahl_badezimmer': anzahl_badezimmer, 'anzahl_zimmer': anzahl_zimmer,
            'anzahl_parkplatz': anzahl_parkplatz, 'baujahr': baujahr, 'grundstuecksflaeche': grundstuecksflaeche,
    ...
    verstaedterung = features['Grad_der_Verstädterung'].to_list()[0]
    verstaedterung_string = 'SELECT Grad_der_Verstädterung_targetenc FROM Encoding_Grad_der_Verstädterung WHERE ' \
                            'Grad_der_Verstädterung=\'' + verstaedterung + '\''
    verstaedterung = np.float32(pd.read_sql_query(verstaedterung_string, con=db_connection).iloc[0][0])
    features['Grad_der_Verstädterung'] = verstaedterung

    soziolage = features['sozioökonomische_Lage'].to_list()[0]
    soziolage_string = 'SELECT sozioökonomische_Lage_targetenc FROM Encoding_sozioökonmische_Lage WHERE ' \
...
st.write(
    ...
    'fundierte Bewertung. Neben informativen Hinweisen zu deiner geographischen Umgebung, stellen wir die im '
...
with Metadaten_plz:
    ...
    Meta_krankenhaus = round(Meta[Meta['plz'] == plz]['Erreichbarkeit Krankenhaus mit Maximalversorgung'].to_list()[0],
                             2)
