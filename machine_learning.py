...
def boolean(imputed_data):
    ...
    imputed_data = imputed_data.assign(
        supermarkt_im_plz_gebiet=(imputed_data['Supermarkt im PLZ Gebiet'] == 'JA').astype(int))
...
def target_encoding(x_train_cat, x_val_cat, y_train):
    ...
    energietyp.to_sql(name='Encoding_energietyp', con=model_training.setup_database(r"Datenbank/ImmoDB.db"),
                      if_exists='replace')
    ...
    energie_effizienzklasse.to_sql(name='Encoding_energie_effizienzklasse',
                                   con=model_training.setup_database(r"Datenbank/ImmoDB.db"), if_exists='replace')

    heizung = x_train_reference[['heizung', 'heizung_targetenc']]
    heizung = heizung.drop_duplicates(subset=['heizung'])
    heizung.to_sql(name='Encoding_heizung', con=model_training.setup_database(r"Datenbank/ImmoDB.db"),
                   if_exists='replace')
    ...
    immobilienart.to_sql(name='Encoding_immobilienart', con=model_training.setup_database(r"Datenbank/ImmoDB.db"),
                         if_exists='replace')
    ...
    grad_der_verstaedterung = x_train_reference[['Grad_der_Verstädterung', 'Grad_der_Verstädterung_targetenc']]
    grad_der_verstaedterung = grad_der_verstaedterung.drop_duplicates(subset=['Grad_der_Verstädterung'])
    grad_der_verstaedterung.to_sql(name='Encoding_Grad_der_Verstädterung',
                                   con=model_training.setup_database(r"Datenbank/ImmoDB.db"), if_exists='replace')

    soziooekonmische_lage = x_train_reference[['sozioökonomische_Lage', 'sozioökonomische_Lage_targetenc']]
    soziooekonmische_lage = soziooekonmische_lage.drop_duplicates(subset=['sozioökonomische_Lage'])
    soziooekonmische_lage.to_sql(name='Encoding_sozioökonmische_Lage',
                                 con=model_training.setup_database(r"Datenbank/ImmoDB.db"), if_exists='replace')
...
# Zusammenführung kategorischer und numerischer Varibalen + Speicherung unter Standard Variablennamen
...
def ml_tests(x_train, x_test, y_train, y_test, imputed_data):
    ...
    # Machine Learning Modelle trainieren und in pckl Dateien speichern, für spätere Verwendung in der Anwendung
    # Teilweise sind die ML-Algorithmen auskommentiert, da die Laufzeit sehr lang wird, wenn man alle ausführt.
    # Dies hängt aber auch maßgeblich von der Rechenleistung des verwendeten PCs ab.
    ...
    print_feature_importances(model=sgbr, data=imputed_data.drop(columns=["angebotspreis"]),
                              save_string='Files/Feature_Importances_Grafiken/sgbr_feature_importances.jpg')
    ...
    print_feature_importances(model=rf, data=imputed_data.drop(columns=["angebotspreis"]),
                              save_string='Files/Feature_Importances_Grafiken/rf_feature_importances.jpg')
    ...
    # final_estimator = xgb.XGBRegressor(objective="reg:squarederror", n_estimators=407, max_depth=4,
    #                                   min_child_weight=2, eta=0.1, subsample=1, colsample_bytree=1, seed=123)
